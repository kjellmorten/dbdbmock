...
class DbdbCouch {
   ...
   * @param {object} options - Query options. filter, max, and first
  ...
  getView (id, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      options = Object.assign({first: 0, max: null, desc: false}, options)
      ...
      if (options.firstKey) {
        const firstKey = (options.filter) ? [].concat(options.filter.split('/'), options.firstKey) : options.firstKey
        // Get index of element with given key
        first = items.findIndex((element) => {
          return (element._key && (JSON.stringify(element._key) === JSON.stringify(firstKey)))
        })
      }

      // Get paged portion of array
      if (options.max) {
        items = items.slice(first, first + options.max)
      } else if (first > 0) {
