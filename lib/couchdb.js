...
const getJsonKeyFromItem = (item) => (item && item._key) ? JSON.stringify(item._key) : null

const getKeyIndex = (key, items, desc, includeNextOnNotFound) => {
  if (items.length === 0) {
    return -1
  }
  ...
  const index = (desc)
    ? items.findIndex((el) => (el._key && (JSON.stringify(el._key) <= jsonKey)))
    : items.findIndex((el) => (el._key && (JSON.stringify(el._key) >= jsonKey)))

  if (index === -1) {
    // If no match, decide whether to return index in the beginning or end of array
    const firstKey = getJsonKeyFromItem(items[0])
    if (firstKey) {
      return (firstKey > jsonKey && !desc) ? ((includeNextOnNotFound) ? 0 : -1) : items.length
    } else {
      return -1
    }
  } else {
    // If an exact match is not found and includeNextOnNotFound is false, return index
    // before the place where a match would have been found
    const itemKey = getJsonKeyFromItem(items[index])
    if (!includeNextOnNotFound && itemKey && (itemKey !== jsonKey)) {
      return index - 1
    }
    return index
  }
  ...
  getView (id, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      if (options.firstKey) {
        ...
        first = getKeyIndex(firstKey, items, options.desc, true)
      }
      ...
      if (options.lastKey) {
        ...
        last = getKeyIndex(lastKey, items, options.desc, false) + 1
      }
